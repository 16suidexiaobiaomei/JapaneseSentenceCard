-- Furigana (ruby-annotated readings) shown above the Japanese sentence
-- on the back of a review card, alongside romaji. Stored pre-generated
-- (same as kana) rather than computed on-device, since it needs the
-- same kuromoji tokenizer used server-side for romaji/kana.
alter table public.romaji_cache add column if not exists furigana text not null default '';
alter table public.cards add column if not exists furigana text;
grant update (furigana) on public.cards to authenticated;
alter table public.shared_tag_cards add column if not exists furigana text;
...
begin
  ...
  insert into public.shared_tag_cards (shared_tag_id, front, romaji, back, kana, furigana, sort_order)
  select v_shared_tag_id, c.front, c.romaji, c.back, c.kana, c.furigana, row_number() over (order by c.created_at)
  from public.cards c
...
begin
  ...
  insert into public.shared_tag_cards (shared_tag_id, front, romaji, back, kana, furigana, sort_order)
  select p_shared_tag_id, c.front, c.romaji, c.back, c.kana, c.furigana, row_number() over (order by c.created_at)
  from public.cards c
  where c.id = any(p_card_ids) and c.user_id = v_uid;
