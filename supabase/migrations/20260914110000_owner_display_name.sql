-- The community browse/detail screens need to show "by <name>" for
-- each shared tag. profiles is intentionally select-locked to its own
-- owner ("profiles: select own"), so a live join at browse time isn't
-- possible without relaxing that RLS and exposing full profile rows to
-- every user — a much bigger privacy surface than this needs. Instead,
-- snapshot just a display name onto shared_tags at share/republish
-- time, same philosophy as shared_tag_cards already being a one-time
-- copy rather than a live link back to the owner's data.
alter table public.shared_tags add column if not exists owner_display_name text not null default 'A learner';
